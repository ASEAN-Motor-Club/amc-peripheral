...
async def actual_discord_poll_creator(bot, question: str, options: list[str], channel_id=None):
    try:
...
async def actual_discord_event_creator(guild, name, desc, loc, start, end, tz_name):
    try:
        ...
        end_dt = datetime.fromisoformat(end).replace(tzinfo=tz) if end else start_dt + timedelta(hours=1)
        event = await guild.create_scheduled_event(
            name=name, description=desc, location=loc, start_time=start_dt, end_time=end_dt,
