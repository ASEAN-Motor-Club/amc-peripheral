...
class UtilsCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        # Context Menus
        self.ctx_menus = [
            app_commands.ContextMenu(name="Record Race Attempt", callback=self.record_race_attempt_context),
            app_commands.ContextMenu(name="Open on Track Editor", callback=self.open_track_context),
        ]
    ...
    async def update_time_embed(self):
        ...
        if self.last_timezone_embed_message is None:
            async for message in channel.history(limit=5):
                if message.author == self.bot.user and message.embeds and message.embeds[0].title == "🕒 World Clock":
            ...
            if self.last_timezone_embed_message:
                ...
                self.last_timezone_embed_message = await channel.send(embed=get_time_embed())
        else:
            try:
                await self.last_timezone_embed_message.edit(embed=get_time_embed())
    ...
    async def race_announcement(self):
        await announce_in_game(self.bot.http_session, "A race is taking place. Please mind the racers.")
    ...
    async def rent_reminders(self):
        ...
        try:
            async with self.bot.http_session.get("https://server.aseanmotorclub.com/api/housing/") as resp:
        ...
        for name, housing in housings.items():
            ...
            if rent_left_seconds is not None and rent_left_seconds < 259200:
                ...
                if general_channel:
                    await general_channel.send(f"Rent reminder: {display_name}, your plot is expiring in {rent_left}")
                if member:
                    try:
                        await member.send(f"Hi {nickname}! Just letting you know that your plot is expiring in {rent_left}")
                    except Exception:
                        pass
    ...
    class ThreadSelectView(discord.ui.View):
        def __init__(self, bot, threads, original_message, *args, **kwargs):
            ...
            options = [discord.SelectOption(label=t.name, value=str(t.id)) for t in threads]
            self.select_menu = discord.ui.Select(placeholder="Select a race thread...", options=options)
        ...
        async def select_callback(self, interaction: discord.Interaction):
            ...
            await interaction.response.send_message(f"Forwarded to '{thread.name}'.", ephemeral=True)
            await interaction.message.delete()

    # --- Commands ---

    @app_commands.command(name="toggle_announcement", description="Toggle announcement")
    async def toggle_announcement_cmd(self, interaction: discord.Interaction, state: str):
    ...
    async def record_race_attempt_context(self, interaction: discord.Interaction, message: discord.Message):
        forum = self.bot.get_channel(1353679733358858321)
        if not isinstance(forum, discord.ForumChannel):
            return await interaction.response.send_message("Not a forum.", ephemeral=True)
        if 'Event JSON' in message.content:
            return await interaction.response.send_message("You must do this before starting.", ephemeral=True)
        if not forum.threads:
            return await interaction.response.send_message("No threads.", ephemeral=True)
        await interaction.response.send_message("Select thread:", view=self.ThreadSelectView(self.bot, reversed(forum.threads), message), ephemeral=True)

    async def open_track_context(self, interaction: discord.Interaction, message: discord.Message):
        ...
        for a in message.attachments:
            try:
                ...
                return await interaction.followup.send(f"**{route_name}**: https://www.aseanmotorclub.com/track?uri={urllib.parse.quote(url, safe='')}")
            except Exception:
                continue
