...
class LinkView(discord.ui.View):
    def __init__(self, url: str, label: str="Open Link"):
        ...
        self.add_item(discord.ui.Button(label=label, style=discord.ButtonStyle.url, url=url))
...
class RadioCog(commands.Cog):
    ...
    async def fetch_knowledge(self):
        ...
        files_messages = [m async for m in files_channel.history(limit=4) if m.attachments]
    ...
    async def fetch_forum_messages(self, forum_channel: discord.ForumChannel, include_dates=False, **history_kwargs):
        acc = ""
...
Use the following information about the game to answer queries. If a user asks a question outside the scope of your knowlege, refer them to the discord channel and other players in the game."""
        ...
        if gen_channel:
            async for m in gen_channel.history(after=datetime.now() - timedelta(hours=hours), oldest_first=True):
                 if not m.author.bot:
        ...
        if game_chat:
            async for m in game_chat.history(after=datetime.now() - timedelta(hours=hours), oldest_first=True):
        ...
        if editorial_channel:
             editorial = await self.fetch_forum_messages(
                editorial_channel,
        ...
        return [
            ...
            {"role": "user", "content": f'# Discord messages (last {hours} hours):\n' + '\n'.join(discord_messages)},
            {"role": "user", "content": f'# In game messages (last {hours} hours):\n' + '\n'.join(game_messages)},
...
'''},
        ...
        for jingle in jingles[:6]:
            audio_bytes = await asyncio.to_thread(
                tts_google,
                discord.utils.remove_markdown(jingle),
                use_markup=True
            )
...
'''},
    ...
    async def request_song(self, youtube_link: str, requester: str, bypass_throttling=False):
        now = datetime.now(self.local_tz)

        # --- Throttling Logic ---
        ten_minutes_ago = now - timedelta(minutes=10)
        self.user_requests.setdefault(requester, [])
        self.user_requests[requester] = [t for t in self.user_requests[requester] if t > ten_minutes_ago]

        five_minutes_ago = now - timedelta(minutes=5)
        requests_last_5_min = sum(1 for t in self.user_requests[requester] if t > five_minutes_ago)
        requests_last_10_min = len(self.user_requests[requester])

        if not bypass_throttling:
            if requests_last_5_min >= 3:
                raise Exception('You have queued too many songs. Please wait a moment. (Limit: 3 songs per 5 minutes)')
            if requests_last_10_min >= 5:
                raise Exception('You have queued too many songs. Please wait a moment. (Limit: 5 songs per 10 minutes)')
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_info_opts) as ydl:
                info_dict = await asyncio.to_thread(ydl.extract_info, search_query, download=False)
            if 'entries' in info_dict and info_dict['entries']:
                info_dict = info_dict['entries'][0]
        except Exception as e:
            raise Exception("Could not find that song. Please try a different name or link.") from e
        ...
        if normalized_title in normalized_queue:
            raise Exception(f'"{title}" has been queued recently. Please choose a different song.')
        
        if duration > 600:
            raise Exception(f'"{title}" is too long ({duration//60}m). Max duration is 10 minutes.')
    ...
    async def compile_playlist(self):
        ...
        if event_songs_channel:
            event_songs_messages = [m async for m in event_songs_channel.history(limit=None) if m.attachments]
            for message in event_songs_messages:
                for attachment in message.attachments:
                    local_path = os.path.join(RADIO_PATH, 'event_songs', attachment.filename)
                    await attachment.save(local_path)

        # Race songs
        race_songs_channel = self.bot.get_channel(RACE_SONGS_CHANNEL)
        if race_songs_channel:
            race_songs_messages = [m async for m in race_songs_channel.history(limit=None) if m.attachments]
            for message in race_songs_messages:
                 for attachment in message.attachments:
                    local_path = os.path.join(RADIO_PATH, 'race_songs', attachment.filename)
                    await attachment.save(local_path)
    ...
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def song_request_cmd(self, interaction: discord.Interaction, song_or_youtube_link: str):
        ...
        try:
            title, _ = await self.request_song(song_or_youtube_link, interaction.user.display_name, bypass_throttling)
            await interaction.followup.send(f'Downloaded, your song "{title}" will be played soon!', ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f'Failed: {e}', ephemeral=True)

    @app_commands.command(name="recompile_playlist", description="Recompile radio playlist")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    @app_commands.checks.has_permissions(administrator=True)
    ...
    async def skip_radio_track(self, interaction: discord.Interaction):
        ...
        self.bot.loop.create_task(asyncio.to_thread(self.lq.skip_current_track, 'song_requests'))
    ...
    async def set_event_mode(self, interaction: discord.Interaction, state: bool):
        ...
        self.bot.loop.create_task(asyncio.to_thread(self.lq._send_command, f'var.set event_mode = {state_str}'))
        self.bot.loop.create_task(asyncio.to_thread(self.lq._send_command, 'var.set race_mode = false'))
    ...
    async def set_race_mode(self, interaction: discord.Interaction, state: bool):
        ...
        state_str = "true" if state else "false"
        self.bot.loop.create_task(asyncio.to_thread(self.lq._send_command, f'var.set race_mode = {state_str}'))
        ...
        async for jingle, jingle_audio in self.generate_jingles_gen():
            ...
            if channel:
                 self.bot.loop.create_task(
                    channel.send(jingle[:2000], file=discord.File(BytesIO(jingle_audio), filename=f'jingle{i}.mp3'))
                 )
            i += 1
    ...
    async def _update_news_logic(self):
        ...
        news_audio = await asyncio.to_thread(tts_google, discord.utils.remove_markdown(news), use_markup=True)
        message = await channel.send(news[:2000], file=discord.File(BytesIO(news_audio), filename='news.mp3'))
    ...
    async def update_current_song_embed(self):
        ...
        filename = None
        try:
            async with self.bot.http_session.get('http://localhost:6001/metadata') as resp:
        ...
        verb = "Previously requested by" if folder == "prev_requests" else "Requested by"
         ...
         if channel_id == RADIO_CHANNEL_ID:
             ...
             if command_match := re.match(r'\*\*(?P<name>.+):\*\* /(?P<command>\w+) (?P<args>.+)', message.content):
                 ...
                 if command == 'song_request':
                     ...
                     self.bot.loop.create_task(asyncio.to_thread(self.lq._send_command, f'var.set event_mode = {args}'))
                 elif command == 'skip':
                     self.bot.loop.create_task(asyncio.to_thread(self.lq.skip_current_track, 'song_requests'))
